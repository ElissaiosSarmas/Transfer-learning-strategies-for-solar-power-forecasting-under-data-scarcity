...
def createModel():
    ...
    x = LSTM(96, activation="relu", return_sequences=False, input_shape=(x_train.shape[1], x_train.shape[2]))(x)
...
def fit_predict_stats(model):
    ...
    optimizer = Adam(learning_rate=0.0001) # low to prevent overfitting
    model.compile(loss='mean_squared_error', optimizer=optimizer, metrics=['mae'])
    es = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=3)
    history = model.fit(x_train, y_train, epochs=20, validation_split=0.2, batch_size=128, verbose=1, shuffle=True, callbacks=[es]).history

    # Save the model
    model.save('PV3_TL3.h5')

    #Load the model
    model = load_model(r'PV3_TL3.h5')
