...
def createModel():
    ...
    model.add(LSTM(48,activation="relu",return_sequences=True, input_shape=(x_train.shape[1], x_train.shape[2])))
    model.add(LSTM(96,activation="relu",return_sequences=False, input_shape=(x_train.shape[1], x_train.shape[2])))
    model.add(Dense(1))
...
def fit_predict_stats(model):
    ...
    print("pred =", hat[0])
    print("pred =", hat[1])
    print("true =", len(true[0]))
    print("true =", len(true[1]))
    print("pred =", len(hat[0]))
    print("pred =", len(hat[1]))
    sns.set_style("darkgrid")
    plt.plot(true[1],color="green")
    plt.plot(hat[1], color="red")
    plt.show()

    pd.DataFrame(true[1]).to_excel("true.xlsx", sheet_name='true') 
    pd.DataFrame(hat[1]).to_excel("hat.xlsx", sheet_name='hat') 
